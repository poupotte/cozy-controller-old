...
//
// ### function installDependenciesPython (callback)
// #### @callback {function} Continuation to pass control back to when complete.
// Installs the required dependencies for the app associated
// with this repository instance on this system.
//
Repository.prototype.installDependenciesPython = function (callback) {
  var self = this;
  if (fs.existsSync(path.join(this.homeDir, 'requirements.txt'))) {
    fs.readFile(path.join(this.homeDir, 'requirements.txt'), function (err, data) {
      if (!err) {
        try {
          pkg = JSON.parse(data);
          pkg.dependencies = pkg.dependencies || {};
      ...
      haibu.common.pip.install(self.homeDir, self.app, callback)
    });
  } else {
    callback(null, null);
  }
