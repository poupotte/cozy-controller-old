...
// Install the `target` dependencies, a requirements.txt
// manifest with potential dependencies.
//
haibu.common.pip.install = function (dir, target, callback) {
  haibu.emit('pip:install', 'info', {
    dir: dir
  })
  var spawnOptions = haibu.getSpawnOptions(target);
  var stderr ='';
  ...
  pip = spawn('pip', args, spawnOptions);

  pip.stdout.on('data', function (data) {
    haibu.emit('pip:install:stdout', 'info', {
      stdout: data
    });
  });
  ...
  pip.stderr.on('data', function (data) {
    stderr += data;
    haibu.emit('pip:install:failure', 'info', {
      data: data
    });
  });

  pip.on('exit', function (code) {
    if (code || code == null) {
      var err = new Error('pip install failed');
      err.code = code;
      err.result = stderr;
      err.blame = {
        message: 'Pip failed to install requirements'
      };
...
//
// ### function build (dir, target, callback)
// #### @dirClient {string} Directory to brunch build into.
// #### @target {Object|Array} application to brunch build.
// #### @callback {function} Continuation to pass control back to when complete.
// Brunch build client of application
//
